...
<div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 clearpaddingl">
	    ...
	    <div class="panel-body">
	  	    <form class="form-horizontal text-center" method="post" action="<?php echo base_url('product/search'); ?>">
				<div class="form-group form-group-sm">
					<label class="col-sm-5 clearpaddingl control-label" for="formGroupInputSmall">Catalogs</label>
				...
				<div class="form-group form-group-sm">
					...
					<div class="col-sm-7" style="padding-left: 0px">
				    	...
				    	<?php for ($i=0; $i < 1400000; $i = $i + 100000) { ?>
				    		<option value="<?php echo $i ?>" <?php if($i==$price_from) echo 'selected'; ?>><?php echo number_format($i); ?> USD</option>
				    	<?php } ?>
						</select>
				    </div>
				</div>
				<div class="form-group form-group-sm">
					...
					<div class="col-sm-7" style="padding-left: 0px">
				    	...
				    	<?php for ($i=100000; $i < 1500000; $i = $i + 100000) { ?>
				    		<option value="<?php echo $i ?>" <?php if($i==$price_to) echo 'selected'; ?>><?php echo number_format($i); ?> USD</option>
				    	<?php } ?>
						</select>
				    </div>
				</div>
				<button class="btn btn-info" type="submit" name='submit'>Sort now</button>
			</form>
	    </div>
	</div>
	...
	<div class="panel panel-info" style="margin-top:10px;margin-bottom:10px;">
	  <div class="panel-body" style="padding:0px;">
	  	<img style="width: 100%" src="<?php echo base_url('upload/qc.png'); ?>" alt="">
	  </div>
	</div>
</div>